...
public class AlarmAlertStopBroadcastReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        showEndAlarmDialog(context, alarm);
     ...
     * @param bean
     */
    private void showEndAlarmDialog(Context context, AlarmBean bean) {
        vibrator = (Vibrator) context.getSystemService(Context.VIBRATOR_SERVICE);
//        playMusicAndVibrate(context, bean);

        AlertDialog.Builder builder = new AlertDialog.Builder(context);
        builder.setTitle("您完成以下任务了吗")
                ...
                .setPositiveButton("完成了", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        new Thread(new Runnable() {
                            @Override
                            public void run() {
                                new AlarmDBSupport(context).updateStatus(bean.getId(), 2);
...
//                                context.sendBroadcast(intent);
                ...
                .setNegativeButton("还没完成哦", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        cancel();
                        new Thread(new Runnable() {
                            @Override
                            public void run() {
                                new AlarmDBSupport(context).updateStatus(bean.getId(), 0);
//                                BusProvider.getInstance().send(new Events.UpdateEvent());
//                                Intent intent = new Intent();
//                                intent.putExtra("update", "update");
//                                intent.setAction("com.jameswong.update");//action与接收器相同
