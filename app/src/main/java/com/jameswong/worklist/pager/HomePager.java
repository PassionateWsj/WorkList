...
import com.jameswong.worklist.ScheduleDetailActivity;
import com.jameswong.worklist.customview.agenda.AgendaAdapter;
import com.jameswong.worklist.customview.agenda.AgendaView;
import com.jameswong.worklist.customview.calendar.CalendarView;
import com.jameswong.worklist.customview.render.DefaultEventRenderer;
import com.jameswong.worklist.customview.render.EventRenderer;
...
import com.jameswong.worklist.utils.ColorUtils;
...
public class HomePager extends BasePager implements StickyListHeadersListView.OnStickyHeaderChangedListener {
    ...
    private List<AlarmBean> mAlllist;
...
//        registerBoradcastReceiver();
    ...
    public void registerBoradcastReceiver() {
        ...
        myIntentFilter.addAction("com.jameswong.update");
        mReceiver = new BroadcastReceiver() {//内部类
            @Override
            public void onReceive(Context context, Intent intent) {
                String update = intent.getStringExtra("update");
                Log.i(TAG, "广播接受到啦~~~~~~~~" + update);
        ...
        mActivity.registerReceiver(mReceiver, myIntentFilter);
    }
...
//                Toast.makeText(mActivity,position+"-"+alarm_id.getText().toString(), Toast.LENGTH_SHORT).show();
        ...
        //将日期与event进行匹配
        CalendarManager.getInstance().loadEvents(mEventList);

        // 添加默认的 Renderer 渲染布局
        addEventRenderer(new DefaultEventRenderer());
//        BusProvider.getInstance().toObserverable().subscribe(event ->{
//            if (event instanceof Events.UpdateEvent) {
//                mEventList = new ArrayList<>();
//                mAlllist = support.getAll();
//                mockList(mEventList, mAlllist);
//                agendaAdapter.updateEvents(mEventList);
//            }
//        });
    }
    ...
    private void mockList(List<CalendarEvent> eventList, List<AlarmBean> beanList) {
        ...
        for (AlarmBean bean : beanList) {
            ...
            boolean isAllday = bean.getIsAllday();
            ...
            if (bean.getAgendaStatus() == 1) {
                long systemCurrentTimeMillis = TimeUtils.getSystemCurrentTimeMillis();
                if (systemCurrentTimeMillis - bean.getEndTimeMillis() > 60000) {

                    support.updateStatus(bean.getId(), 0);
                    bean.setAgendaStatus(0);
                }
            }

            BaseCalendarEvent event1 = new BaseCalendarEvent(bean.getId(), bean.getTitle(), bean.getDescription(), bean.getLocal(),
                    ContextCompat.getColor(mActivity, colorId), startTime1, endTime1, isAllday, startAndEndTime, bean.getAgendaStatus(),bean.getStartTimeMillis(),bean.getEndTimeMillis());
