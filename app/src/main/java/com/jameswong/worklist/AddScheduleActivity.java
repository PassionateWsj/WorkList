...
import com.jameswong.worklist.alarmremind.SendAlarmBroadcast;
import com.jameswong.worklist.alarmsetactivity.SetAlarmTimeActivity;
import com.jameswong.worklist.alarmsetactivity.SetAlarmToneActivity;
import com.jameswong.worklist.alarmsetactivity.SetColorActivity;
import com.jameswong.worklist.alarmsetactivity.SetLocalActivity;
import com.jameswong.worklist.alarmsetactivity.SetRePlayActivity;
import com.jameswong.worklist.database.AlarmDBSupport;
import com.jameswong.worklist.utils.ColorUtils;
import com.jameswong.worklist.utils.DateHelper;
...
public class AddScheduleActivity extends AppCompatActivity {
    ...
    void saveAlarmBeanToDB() {
        ...
        //设置是否全天
        if (isAllDay) {
            alarmBean.setIsAllday(true);
        } else {
            alarmBean.setIsAllday(false);
        }
        ...
        alarmBean.setAgendaStatus(0);

        String startDateString = alarmBean.getStartDateString();
        Log.i(TAG, "startDateString:::" + startDateString);
        alarmBean.setStartTimeMillis(TimeUtils.getTimeMillisByString(startDateString));
        alarmBean.setEndTimeMillis(TimeUtils.getTimeMillisByString(alarmBean.getEndDateString()));
        if (id == 0) {
...
//            System.out.println("更新的数据:" + alarmBean.toString());
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (getIntent().getStringExtra("type").equals("DetailToAdd")) {
            ...
            isAllDay = bean.getIsAllday();
            sw_all_day.setChecked(isAllDay);
            if (isAllDay) {
                alarm_start_time.setVisibility(View.GONE);
                alarm_end_time.setVisibility(View.GONE);
            } else {
    ...
    private void getStartTimePickerDialog() {
        ...
        mStartTimePicker = new TimePickerDialog(this, new TimePickerDialog.OnTimeSetListener() {
            ...
            public void onTimeSet(TimePicker view, int hourOfDay, int minute) {
                ...
                alarmBean.setStartTimeMillis(TimeUtils.getTimeMillisByString(alarmBean.getStartDateString()));
            }
    ...
    private void getEndTimePickerDialog() {
        Calendar calendar = Calendar.getInstance();
        calendar.setTimeInMillis(System.currentTimeMillis() + 1500000);
    ...
    private Calendar getToDay() {
        ...
        today.setTimeInMillis(System.currentTimeMillis() + 300000);
        return today;
    }
